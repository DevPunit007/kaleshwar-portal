                ...
                <div class="card-header">
                    <div class="row">
                        <div class="col mr-auto">
                            <h5 class="backend-title mt-2">Participants with User details for {{$event->eventDetails[0]->title}}</h5>
                        </div>
                    ...
                    <div class="table-responsive d-print-table">
                        <table id="booking-list-table" class="display">
                            ...
                            <tr>
                                ...
                                <th>Birthday</th>
                                <th class="select-filter-row">Ashram visited</th>
                                <th>Attended IE 2011</th>
                                <th>Bookings</th>
                                <th>Ticket</th>
                            ...
                            @foreach($bookings as $booking)
                                <tr>
                                    ...
                                    @if($booking->user->personalInformation)<td data-order="{{date_format(date_create($booking->user->personalInformation->date_of_birth), 'Y-m-d')}}">{{date_format(date_create($booking->user->personalInformation->date_of_birth), 'd.m.Y')}}</td>@else <td></td>@endif
                                    <td>{{$booking->user->spiritualHistory->AshramVisitedName ??''}}</td>
                                    <td>{{$booking->user->ashramData->AttendIe2011Name ??''}}</td>

                                    <td>{{$booking->user->BookingsCount ??''}}</td>
