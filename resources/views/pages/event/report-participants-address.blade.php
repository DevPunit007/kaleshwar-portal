                ...
                <div class="card-header">
                    <div class="row">
                        <div class="col mr-auto">
                            <h5 class="backend-title mt-2">Participants with Address for {{$event->eventDetails[0]->title}}</h5>
                        </div>
                    ...
                    <div class="table-responsive d-print-table">
                        <table id="booking-list-table" class="display">
                            ...
                            <tr>
                                ...
                                <th>Zip</th>
                                <th>State</th>
                                <th class="select-filter-row">Country</th>
                                <th>Ticket</th>
                            ...
                            @foreach($bookings as $booking)
                                <tr>
                                    ...
                                    <td class="dt-body-nowrap"><a href="{{ route('user-edit-basic', ['language' => app()->getLocale(), 'id' => $booking->user->id]) }}">{{$booking->user->first_name}} {{$booking->user->last_name}}</a><a href="https://webmail.your-server.de/imp/dynamic.php?page=compose&type=new&to={{$booking->user->email}}" target="_blank">
                                            ...
                                            </svg></a><br>{{__('countries.'.$booking->user->contactInformation->country)}}</td>
                                    <td>{{$booking->eventSectionDetail->TitleShort ??''}}</td>
                                    <td>{{$booking->user->contactInformation->address_street ??''}} {{$booking->user->contactInformation->address_no ??''}} <br>{{$booking->user->contactInformation->address_supplements ? '(' . $booking->user->contactInformation->address_supplements . ')' : ''}}</td>
                                    <td>{{$booking->user->contactInformation->city ??''}}</td>
                                    <td>{{$booking->user->contactInformation->zip ??''}}</td>
                                    <td>{{$booking->user->contactInformation->state ??''}}</td>
                                    <td>{{__('countries.'.$booking->user->contactInformation->country)}}</td>
