                                ...
                                <div class="tab-pane fade active show" id="booking-basic-form" role="tabpanel" aria-labelledby="booking-basic-tab">
                                    <form class="enable-able-form" method="post" action="{{ route('edit-booking', ['language' => app()->getLocale(), 'id' => $booking->id]) }}" enctype="multipart/form-data">@csrf
                                        <div class="button-bar">
                                            <div class="btn-group">
                                                <button type="button" class="btn btn-outline-primary dropdown-toggle btn-header" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                    Actions
                                                </button>
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    <a class="dropdown-item @if($booking->status == 2 || $booking->status == 3 || $booking->status == 4 || $booking->status == 5) disabled @endif" href="#">Move to the waitlist</a>
                                                    <a class="dropdown-item @if($booking->status == 2 || $booking->status == 3 || $booking->status == 5) disabled @endif" @if($booking->bookingDetail && $booking->bookingDetail->arrival_ashram && $booking->bookingDetail->departure_ashram) href="{{ route('email-booking-accepted', ['language' => app()->getLocale(), 'id' => $booking->id]) }}" onclick="return confirm('Do you sure you want to change the status to >Accepted< ?');" @else onclick="alert('You need arrival and departure date to accept the registration !! ');" @endif>Accept the registration</a>
                                                    <a class="dropdown-item @if($booking->status == 1 || $booking->status == 3 || $booking->status == 4 || $booking->status == 5) disabled @endif" href="{{ route('email-booking-no-payment', ['language' => app()->getLocale(), 'id' => $booking->id]) }}" onclick="return confirm('Do you sure you want to change the status to >Free< ?');">No payment necessary</a>
                                                    <a class="dropdown-item @if($booking->status == 1 || $booking->status == 3 || $booking->status == 4 || $booking->status == 5) disabled @endif" @if($booking->event_section_price) href="{{ route('email-booking-payment-confirmation', ['language' => app()->getLocale(), 'id' => $booking->id]) }}" onclick="return confirm('Do you sure you want to change the status to >Paid< ?');" @else onclick="alert('You can not confirm a payment without a booking price!')" @endif>Confirm the payment</a>
                                                    <a class="dropdown-item @if($booking->status == 3) disabled @endif" href="{{ route('cancel-booking', ['language' => app()->getLocale(), 'id' => $booking->id]) }}">Cancel the booking</a>
                                                    <div class="dropdown-divider"></div>
                                                    <a class="dropdown-item @if($booking->status == 1 || $booking->status == 3 || $booking->status == 4 || $booking->status == 5) disabled @endif" @if($booking->event_section_price) href="{{ route('email-booking-payment-reminder', ['language' => app()->getLocale(), 'id' => $booking->id]) }}" onclick="return confirm('Do you sure you want to send the reminder to the Participant ?');" @else onclick="alert('You can not send a payment reminder without a booking price!')" @endif>Send payment reminder</a>
                                                </div>
                                            </div>
                                        ...
                                        <div class="card-body">
                                                ...
                                                <div class="form-group col-lg-5">
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        <div class="input-group-append">
                                                            <span class="input-group-text" id="addon"><a href="{{ route('user-edit-basic', ['language' => app()->getLocale(), 'id' => $user->id]) }}"> > </a></span>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="form-row">
                                                ...
                                                <div class="form-group col-lg-3">
                                                    ...
                                                    <div class="input-group">
                                                        <input name="event_section_price" type="text" class="form-control floatNumber" id="event_section_price" value="{{$booking->event_section_price ??''}}" aria-describedby="addon" disabled>
                                                        <div class="input-group-append">
                                                            <select class="input-group-text form-control" name="currency" id="currency" disabled>
                                                                <option value="" @if($booking->currency == "") selected @endif>./.</option>
                                                                <option value="1" @if($booking->currency == 1) selected @endif>$</option>
                                                                <option value="2" @if($booking->currency == 2) selected @endif>€</option>
                                                            </select>
        {{--                                                    <span class="input-group-text" id="addon">@switch($booking->currency) @case('1') $ @break @case('2') € @break @default ./. @break @endswitch</span>--}}
                                                        </div>
                                            ...
                                            <div class="form-row">
                                                <div class="form-group col-12">
                                                    ...
                                                    <textarea id="notes" name="notes" class="form-control" rows="2" maxlength="190" disabled>{{$booking->notes ??''}}</textarea>
                                                </div>
                                            </div>
                                            <div class="form-row">
                                                <div class="form-group col-12">
                                                    <label for="booking_message">Booking message from the user °</label>
                                                    <textarea id="booking_message" class="form-control" rows="3" readonly>{{$booking->booking_message ??''}}</textarea>
                                                </div>
                                            </div>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="form-row">
                                                <div class="form-group col-sm-12">
                                                    ...
                                                    <textarea class="form-control" id="emergency_contact" name="emergency_contact" maxlength="190" rows="2" @if($booking->bookingDetail) disabled @endif>{{$booking->bookingDetail->emergency_contact ??''}}</textarea>
                                                </div>
                                            </div>
