    ...
    <div class="card">
            ...
            <div class="row">
                <div class="col mr-auto">
                    <h5 class="backend-title mt-2">
                        @if($list_status == 'current')
                            List of bookings of the last 2 months
                        @elseif($list_status == 'open')
                            List of bookings with open status
                        @else
                ...
                <div class="col-auto text-right d-print-none">
                    ...
                    <div class="dropdown-menu">
                        <a class="dropdown-item" href="{{ route('booking-list-current', app()->getLocale()) }}">Current bookings</a>
                        <a class="dropdown-item" href="{{ route('booking-list-open', app()->getLocale()) }}">Open bookings</a>
                        <div class="dropdown-divider"></div>
                        <a class="dropdown-item" href="{{ route('booking-list', app()->getLocale()) }}">All bookings</a>
                    </div>

                    <button onclick="print();" type="button" class="btn btn-outline-info btn-header">Print</button>
                    <button onclick="alert('is coming soon');" type="button" class="btn btn-outline-success btn-header">Add New Booking</button>
                </div>
            </div>
        </div>
        <div class="card-body">
                ...
                <table id="booking-list-table" class="display">
                    <thead>
                        <tr>
                            ...
                            <th>Booking at</th>
                            <th>Participant</th>
                            <th class="select-filter-row">Program</th>
        ...
        <div class="card-footer">
            <div class="text-center">
                @if($list_status == 'current')
                    The list contains all current bookings after {{\Carbon\Carbon::today()->subMonth(2)->format('d.m.Y')}}
                @elseif($list_status == 'open')
                    The list contains all bookings with open status ("Registered", "Accepted" and "Wait")
                @else
                    The list contains all bookings since portal was launched in August 2020.
                @endif
            </div>
        </div>
    </div>
